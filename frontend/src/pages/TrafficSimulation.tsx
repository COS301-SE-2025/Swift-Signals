...
// Update the import path to the correct location of SimulationUI
import { SimulationUI } from '../components/SimulationUI';

// --- Data Interfaces & Helpers ---
interface Node { id: string; x: number; y: number; type: string; }
interface Edge { id:string; from: string; to: string; speed: number; lanes: number; }
interface Position { time: number; x: number; y: number; speed: number; }
interface VehicleData { id: string; positions: Position[]; }
interface TrafficLightPhase { duration: number; state: string; }
interface TrafficLightState { time: number; state: string; }
interface TrafficLightData {
    id: string;
    phases: TrafficLightPhase[];
    states?: TrafficLightState[]; // This is calculated at runtime
}
interface Connection {
    ...
    fromLane: number;
    toLane: number;
    tl: string; // This field is used as the linkIndex
}
interface SimulationData {
    intersection: { nodes: Node[]; edges: Edge[]; trafficLights?: TrafficLightData[]; connections: Connection[];}
    vehicles: VehicleData[];
}
const findNodeById = (nodes: Node[], id: string): Node | null => nodes.find(n => n.id === id) || null;
const lerp = (start: number, end: number, t: number): number => start * (1 - t) + end * t;
const realisticCarColors = [0xEAEAEA, 0xB0B0B0, 0x4B8BBE, 0xD44D5C, 0x3E8E7E, 0xF2A34F, 0x6A6A6A, 0x99B898];
const getRandomCarColor = () => realisticCarColors[Math.floor(Math.random() * realisticCarColors.length)];
...
const Roads: FC<{ edges: Edge[]; nodes: Node[]; center: THREE.Vector2 }> = ({ edges, nodes, center }) => {
    return (
        <group>
            {edges.map(edge => {
                ...
                if (!fromNode || !toNode) {
                    console.warn(`Missing nodes for edge ${edge.id}: from=${edge.from}, to=${edge.to}`);
                    return null;
                }
                ...
                const roadWidth = edge.lanes * 10; // Total road width
                const laneWidth = roadWidth / edge.lanes; // Width of each lane
                const position = new THREE.Vector3((start.x + end.x) / 2, 0, (start.y + end.y) / 2);

                // Debug: Log edge details to verify lane count
                console.log(`Edge ${edge.id}: lanes=${edge.lanes}, roadWidth=${roadWidth}, laneWidth=${laneWidth}, length=${length}`);
                ...
                return (
                    <group key={edge.id} position={position} rotation={[0, -angle, 0]}>
                        ...
                        {/* Lane dividers or center line */}
                        {Array.from({ length: edge.lanes === 1 ? 1 : edge.lanes - 1 }).map((_, laneIndex) => {
                            const offset = edge.lanes === 1 ? 0 : (laneIndex + 1) * laneWidth - roadWidth / 2; // Center for single-lane, between lanes for multi-lane
                            console.log(`Divider for edge ${edge.id}, index ${laneIndex}: offset=${offset}`);
                            return (
                                <group key={`divider-${edge.id}-${laneIndex}`}>
                                    {Array.from({ length: Math.floor(length / 10) }).map((_, i) => (
                                        ...
                                        >
                                            <planeGeometry args={[4, 0.2]} /> {/* Thinner dashes: width reduced to 0.3 */}
...
const Vehicle: FC<{
    vehicleData: VehicleData; simulationTime: number; offset: THREE.Vector2; color: number;
}> = ({ vehicleData, simulationTime, offset, color }) => {
    const vehicleRef = useRef<THREE.Group>(null);
    useFrame(() => {
        if (!vehicleRef.current || vehicleData.positions.length < 2) return;
        let currentPos: Position | null = null, nextPos: Position | null = null;
        for (let i = 0; i < vehicleData.positions.length - 1; i++) {
            if (simulationTime >= vehicleData.positions[i].time && simulationTime < vehicleData.positions[i + 1].time) {
                currentPos = vehicleData.positions[i]; nextPos = vehicleData.positions[i + 1]; break;
            }
        }
        if (currentPos && nextPos) {
            vehicleRef.current.visible = true;
            const timeDiff = nextPos.time - currentPos.time;
            const progress = timeDiff > 0 ? (simulationTime - currentPos.time) / timeDiff : 0;
            const rawX = lerp(currentPos.x, nextPos.x, progress), rawY = lerp(currentPos.y, nextPos.y, progress);
            const alignedX = rawX - offset.x, alignedY = rawY - offset.y;
            ...
            if (Math.abs(dx) > 0.01 || Math.abs(dy) > 0.01) {
                vehicleRef.current.lookAt(nextPos.x - offset.x, 0, nextPos.y - offset.y);
            }
            
        } else {
            // FIX: Hide the vehicle if it's no longer on a valid path segment.
            // This occurs before it starts or after it has finished its journey.
            vehicleRef.current.visible = false;
        }
    });
    return (
        <group ref={vehicleRef} castShadow>
            <mesh castShadow position={[0, 0.6, 0]}><boxGeometry args={[2.4, 1.2, 4.8]} /><meshStandardMaterial color={color} metalness={0.6} roughness={0.3}/></mesh>
            <mesh position={[0, 1.5, -0.5]}><boxGeometry args={[2.2, 0.6, 2.5]} /><meshStandardMaterial color={0x111111} metalness={0.2} roughness={0.5} /></mesh>
        </group>
    );
};
...
const TrafficLightVisual: FC<TrafficLightVisualProps> = ({ state }) => {
    const s = state ? state.toLowerCase() : 'r';

    // Define colors for each state - FIXED
    const redColor = 0xff0000;      // Correct: Bright Red
    const yellowColor = 0xffff00;   // Correct: Bright Yellow
    const greenColor = 0x00ff00;    // Correct: Bright Green

    // Determine active light and color for debugging
    const activeLight = s === 'r' ? 'red' : s === 'y' || s === 'u' ? 'yellow' : 'green';
    const activeColor = s === 'r' ? redColor : s === 'y' || s === 'u' ? yellowColor : greenColor;

    // Debugging: Log active light and its color
    console.log({
        state: s,
        activeLight,
        activeColor: activeColor.toString(16),
        housing: { position: '[0, 2, -0.1]', material: 'meshStandardMaterial, color=0x222222' },
        note: 'Only active light rendered; housing renderOrder=0, active light renderOrder=1, pointLight renderOrder=2'
    });
    ...
    const lightInfoByRoad = useMemo(() => {
        ...
        for (const roadId in connectionsByRoad) {
            ...
            const roadStates = roadConnections.map(conn => {
                const stateChar = (currentStateString[parseInt(conn.tl, 10)] || 'r').toLowerCase();
                return stateChar;
            });
            
            let finalStateChar = 'r'; // Default to red
            if (roadStates.includes('g')) {
                finalStateChar = 'g';
            ...
            if (edge) {
                roadMap.set(roadId, { stateChar: finalStateChar, edge });
            }
        }
        return roadMap;
        ...
        <group>
            {Array.from(lightInfoByRoad.entries()).map(([roadId, { stateChar, edge }]) => {
                ...
                const lightPos = new THREE.Vector3(
                    end.x - dir.x * 5, 0, end.y - dir.y * 5
                ).add(new THREE.Vector3(perp.x * offsetDistance, 0, perp.y * offsetDistance));
...
const SimulationController: FC<{
    simulationData: SimulationData; isPlaying: boolean; speed: number;
    offset: THREE.Vector2; roadCenter: THREE.Vector2; onTimeUpdate: (time: number) => void;
    roadDirections: { [key: string]: string };
...
const TrafficSimulation: FC = () => {
 ...
 useEffect(() => {
    ...
    // --- FIX: Reordered phase generation to match vehicle data ---
    // The original code started with the E/W green phase, but the vehicle
    // data shows N/S traffic moving first. We now generate the N/S
    // phases first to synchronize the lights with the cars.
    ...
    const processedTrafficLights = data.intersection.trafficLights.map(light => {
    let time = 0;
 ...
 if (!simulationData) return { roadCenter: new THREE.Vector2(0, 0), offset: new THREE.Vector2(0, 0) };
 const bounds = {
 ...
 simulationData.vehicles.forEach(v => v.positions.forEach(p => {
 ...
 const roadCenter = new THREE.Vector2((bounds.road.minX + bounds.road.maxX) / 2, (bounds.road.minY + bounds.road.maxY) / 2);
 const vehicleCenter = new THREE.Vector2((bounds.vehicle.minX + bounds.vehicle.maxX) / 2, (bounds.vehicle.minY + bounds.vehicle.maxY) / 2);
 ...
 if (!simulationData) {
 return <div style={{ height: '100vh', display: 'grid', placeContent: 'center', backgroundColor: '#3d3d3d', color: 'white' }}>Loading Simulation...</div>;
 }
 ...
 <MapControls />
 <OrthographicCamera makeDefault position={[0, 100, 0]} zoom={5} />
