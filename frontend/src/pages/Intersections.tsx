 ...
 const handleChange = (
  ...
  setFormData((prev) => {
    ...
    if (keys.length > 1) {
      ...
      (currentLevel as Record<string, unknown>)[finalKey] = isNumber ? parseInt(value, 10) || 0 : value;
    } else {
...
const Intersections = () => {
 ...
 const handleDeleteConfirm = async () => {
  ...
  try {
   ...
   setError(err instanceof Error ? err.message : "Failed to delete intersection");
  } finally {
   setIsDeleting(false);
  }
 };
