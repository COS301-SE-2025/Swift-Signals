...
const LocationMarker: React.FC<{
  ...
  const findNearestIntersection = async (lat: number, lon: number): Promise<Intersection | null> => {
    try {
      setIsSnapping?.(true);
      setIsLoading(true);

      // Search radius in degrees (approximately 500m)
      const radius = 0.005;
      const bbox = `${lat - radius},${lon - radius},${lat + radius},${lon + radius}`;

      // Overpass query to find road intersections near the clicked point
      const overpassQuery = `
        [out:json][timeout:30];
        (
      ...
      if (!data.elements || data.elements.length === 0) {
        console.log('No roads found near clicked location');
        return null;
      }

      // Find intersections by looking for shared nodes between ways
      const ways = data.elements.filter((el: any) => el.type === 'way' && el.tags?.name);
      const nodes = data.elements.filter((el: any) => el.type === 'node');
      
      if (ways.length < 2) {
        console.log('Not enough named roads found for intersection');
        return null;
      }
      ...
      // Find intersection nodes (shared by 2+ ways with different names)
      const intersections: Intersection[] = [];
      
      for (const [nodeId, nodeWays] of nodeWaysMap.entries()) {
        if (nodeWays.length >= 2) {
          // Get unique road names at this node
          const uniqueRoads = [...new Set(nodeWays.map((way: any) => way.tags?.name).filter(Boolean))];
          ...
          if (uniqueRoads.length >= 2) {
            ...
            if (nodeCoords) {
              ...
              const distance = Math.sqrt(
                Math.pow(nodeCoords.lat - lat, 2) + Math.pow(nodeCoords.lon - lon, 2)
              );
              
              intersections.push({
                lat: nodeCoords.lat,
                lon: nodeCoords.lon,
                roads: uniqueRoads.slice(0, 2), // Take first two roads
                intersection: `${uniqueRoads[0]} & ${uniqueRoads[1]}`,
                distance
              } as Intersection & { distance: number });
            }
          }
        }
      }
      ...
      intersections.sort((a: any, b: any) => a.distance - b.distance);
      
      if (intersections.length > 0) {
        const nearest = intersections[0];
        console.log('Found nearest intersection:', nearest);
        return nearest;
      }
  ...
  useMapEvents({
    async click(e) {
      ...
      try {
        ...
        const nearestIntersection = await findNearestIntersection(e.latlng.lat, e.latlng.lng);
        ...
        if (nearestIntersection) {
          ...
          setCoordinates(`${nearestIntersection.lat.toFixed(6)}, ${nearestIntersection.lon.toFixed(6)}`);
          
          console.log('Snapped to intersection:', nearestIntersection.intersection);
        } else {
      ...
      {isLoading && (
        <div className="absolute top-2 left-2 bg-white dark:bg-gray-800 px-3 py-1 rounded-md shadow-md z-10">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-700 dark:text-gray-300">Finding nearest intersection...</span>
          </div>
        </div>
      )}
    </>
...
const StreetSearchComponent: React.FC<{
  ...
  const [firstStreetSuggestions, setFirstStreetSuggestions] = useState<Street[]>([]);
  const [secondStreetSuggestions, setSecondStreetSuggestions] = useState<Street[]>([]);
  const [showFirstDropdown, setShowFirstDropdown] = useState(false);
  const [isLoadingFirst, setIsLoadingFirst] = useState(false);
  const [isLoadingSecond, setIsLoadingSecond] = useState(false);
  const [selectedFirstStreet, setSelectedFirstStreet] = useState<Street | null>(null);
  ...
  const searchStreets = async (query: string): Promise<Street[]> => {
    ...
    try {
      ...
      const strategies = [
        ...
        () => {
          const hasStreetSuffix = /\b(street|road|avenue|drive|lane|way|boulevard|crescent)\b/i.test(cleanQuery);
          if (!hasStreetSuffix) {
  ...
  const searchNominatimStreets = async (query: string, type: string = 'road'): Promise<Street[]> => {
    try {
  ...
  const processSouthAfricanStreets = (data: any[]): Street[] => {
    if (!Array.isArray(data)) return [];
    
    const processed = data
      .filter((item: any) => {
        // Must be in South Africa
        const isInSA = item.address?.country_code === 'za' || 
                      item.display_name?.includes('South Africa');
        
        // Must be a road/highway
        const isRoad = item.class === 'highway' || 
                      item.type === 'road' ||
                      item.address?.road ||
                      /\b(street|road|avenue|drive|lane|way|boulevard|crescent|highway)\b/i.test(item.display_name);
        
        // Filter out non-road types
        const validType = !['hamlet', 'village', 'isolated_dwelling', 'city', 'town', 'suburb'].includes(item.type);
      ...
      .map((item: any) => {
        ...
        return {
          ...
          province: item.address?.state || item.address?.province || 'South Africa',
          lat: coordinates.lat,
          lon: coordinates.lon,
          osmId: item.osm_id,
      ...
      .filter((street: Street, index: number, self: Street[]) => 
  ...
  const extractStreetName = (item: any): string => {
  ...
  const getCoordinates = (item: any): { lat: number | null; lon: number | null } => {
  ...
  const findIntersectingStreets = async (street: Street): Promise<Street[]> => {
    ...
    try {
      ...
      if (street.lat && street.lon) {
        const coordResults = await findIntersectionsAtCoordinates(street.lat, street.lon, street.name);
        if (coordResults.length > 0) {
          console.log('Found intersections via coordinates:', coordResults.length);
          return coordResults;
        }
      }
      ...
      if (overpassResults.length > 0) {
        console.log('Found intersections via Overpass:', overpassResults.length);
        return overpassResults;
      }

      // Strategy 3: Search Nominatim for intersection mentions
      const nominatimResults = await searchIntersectionMentions(street.name, street.city);
      if (nominatimResults.length > 0) {
        console.log('Found intersections via Nominatim search:', nominatimResults.length);
  ...
  const findIntersectionsAtCoordinates = async (lat: number, lon: number, streetName: string): Promise<Street[]> => {
    try {
      console.log(`Finding intersections at coordinates: ${lat}, ${lon} for ${streetName}`);
  ...
  const findIntersectionsViaOverpass = async (street: Street): Promise<Street[]> => {
    try {
      const searchArea = street.city && street.city !== 'South Africa' ? 
  ...
  const findAllIntersectingWays = async (targetWays: any[], streetName: string): Promise<Street[]> => {
    try {
  ...
  const extractIntersectionsFromOverpassData = (elements: any[], originalStreet: string): Street[] => {
    ...
    const intersectingWays = elements.filter(el => 
      el.type === 'way' && 
      el.tags && 
      el.tags.name && 
      el.tags.highway &&
    ...
    console.log(`Found ${intersectingWays.length} potentially intersecting ways`);
    ...
    intersectingWays.forEach(way => {
      if (way.tags.name && isValidStreetName(way.tags.name)) {
        ...
        const centerLat = bounds.minlat && bounds.maxlat ? (bounds.minlat + bounds.maxlat) / 2 : null;
        const centerLon = bounds.minlon && bounds.maxlon ? (bounds.minlon + bounds.maxlon) / 2 : null;
    ...
    const uniqueStreets = intersectingStreets
      .filter((street, index, self) => 
        self.findIndex(s => s.name.toLowerCase() === street.name.toLowerCase()) === index
      )
  ...
  const searchIntersectionMentions = async (streetName: string, city?: string): Promise<Street[]> => {
    try {
      const searchQueries = [
      ...
      for (const query of searchQueries) {
        ...
        if (response.ok) {
          const data = await response.json();
          const intersections = extractIntersectionsFromDisplayNames(data, streetName);
          if (intersections.length > 0) {
  ...
  const extractIntersectionsFromDisplayNames = (data: any[], originalStreet: string): Street[] => {
    if (!Array.isArray(data)) return [];
    ...
    // Remove duplicates
    const uniqueStreets = intersectingStreets.filter((street, index, self) => 
      self.findIndex(s => s.name.toLowerCase() === street.name.toLowerCase()) === index
    );
  ...
  const isValidStreetName = (name: string): boolean => {
    ...
    const streetSuffixes = /(street|road|avenue|drive|lane|way|boulevard|crescent|place|close|court|grove|gardens|park|square|circle|terrace|highway|route)$/i;
  ...
  const getCommonSouthAfricanStreets = (query: string): Street[] => {
    const commonStreets: Street[] = [
      { name: 'Main Road', city: 'Various Cities', province: 'All Provinces', lat: null, lon: null },
      { name: 'Church Street', city: 'Various Cities', province: 'All Provinces', lat: null, lon: null },
      { name: 'Voortrekker Road', city: 'Cape Town', province: 'Western Cape', lat: null, lon: null },
      { name: 'Jan Smuts Avenue', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
      { name: 'Nelson Mandela Drive', city: 'Various Cities', province: 'All Provinces', lat: null, lon: null },
      { name: 'Commissioner Street', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
      { name: 'Long Street', city: 'Cape Town', province: 'Western Cape', lat: null, lon: null },
      { name: 'Smith Street', city: 'Durban', province: 'KwaZulu-Natal', lat: null, lon: null },
      { name: 'Paul Kruger Street', city: 'Pretoria', province: 'Gauteng', lat: null, lon: null },
      { name: 'William Nicol Drive', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
      { name: 'Barry Hertzog Avenue', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
      { name: 'Louis Botha Avenue', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
      { name: 'Oxford Road', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
      { name: 'Adderley Street', city: 'Cape Town', province: 'Western Cape', lat: null, lon: null },
      { name: 'West Street', city: 'Durban', province: 'KwaZulu-Natal', lat: null, lon: null },
      { name: 'Schoeman Street', city: 'Pretoria', province: 'Gauteng', lat: null, lon: null },
      { name: 'Simon Vermooten Road', city: 'Pretoria', province: 'Gauteng', lat: null, lon: null },
      { name: 'Beyers Naude Drive', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null }
    ];
  ...
  const getContextualIntersectingStreets = (street: Street): Street[] => {
    ...
    const knownIntersections: { [key: string]: Street[] } = {
      'simon vermooten road': [
        { name: 'Jan Shoba Street', city: 'Pretoria', province: 'Gauteng', lat: null, lon: null },
        { name: 'Zambezi Drive', city: 'Pretoria', province: 'Gauteng', lat: null, lon: null },
        { name: 'Solomon Mahlangu Drive', city: 'Pretoria', province: 'Gauteng', lat: null, lon: null },
        { name: 'Lynnwood Road', city: 'Pretoria', province: 'Gauteng', lat: null, lon: null },
        { name: 'Atterbury Road', city: 'Pretoria', province: 'Gauteng', lat: null, lon: null }
      ],
      'jan smuts avenue': [
        { name: 'Rosebank Road', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
        { name: 'Oxford Road', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
        { name: 'Barry Hertzog Avenue', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
        { name: 'Empire Road', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null }
      ],
      'william nicol drive': [
        { name: 'Republic Road', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
        { name: 'Witkoppen Road', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
        { name: 'Main Road', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null },
        { name: 'Bryanston Drive', city: 'Johannesburg', province: 'Gauteng', lat: null, lon: null }
      ],
      'main road': [
        { name: 'Church Street', city: street.city || 'Various', province: street.province || '', lat: null, lon: null },
        { name: 'Market Street', city: street.city || 'Various', province: street.province || '', lat: null, lon: null },
        { name: 'Station Road', city: street.city || 'Various', province: street.province || '', lat: null, lon: null }
      ]
    };

    // Check for known intersections
    for (const [knownStreet, intersections] of Object.entries(knownIntersections)) {
      if (streetName.includes(knownStreet) || knownStreet.includes(streetName)) {
        return intersections;
      }
    }
    ...
    const genericIntersections: Street[] = [
      { name: 'Main Road', city: street.city || 'South Africa', province: street.province || '', lat: null, lon: null },
      { name: 'Church Street', city: street.city || 'South Africa', province: street.province || '', lat: null, lon: null },
      { name: 'Market Street', city: street.city || 'South Africa', province: street.province || '', lat: null, lon: null },
      { name: 'Station Street', city: street.city || 'South Africa', province: street.province || '', lat: null, lon: null }
    ];

    return genericIntersections.filter(s => 
      s.name.toLowerCase() !== streetName
    );
  };
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (firstStreetRef.current && !firstStreetRef.current.contains(event.target as Node)) {
        setShowFirstDropdown(false);
      }
    };
      ...
      <div className="relative" ref={firstStreetRef}>
        ...
        <div className="relative">
          <input
            ...
            onFocus={() => firstStreetSuggestions.length > 0 && setShowFirstDropdown(true)}
                ...
                >
                  ...
                  {street.city && street.city !== 'Various Cities' && (
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                      {street.city}{street.province && street.province !== 'South Africa' && `, ${street.province}`}
                    </div>
                  )}
                </button>
              ))
        ...
        <div className="relative" ref={secondStreetRef}>
          ...
          <div className="relative">
            <select
              value={secondStreet}
              onChange={(e) => {
                const selectedStreet = secondStreetSuggestions.find(s => s.name === e.target.value);
                if (selectedStreet) {
            ...
            >
              ...
              {!isLoadingSecond && secondStreetSuggestions.map((street, index) => (
                <option key={index} value={street.name}>
                  {street.name}
                  {street.city && street.city !== 'Various Cities' && street.city !== 'South Africa' && ` - ${street.city}`}
                  {street.province && street.province !== 'All Provinces' && street.province !== 'South Africa' && street.province !== '' && `, ${street.province}`}
                </option>
              ))}
            </select>
            <ChevronDown 
          ...
          {!isLoadingSecond && secondStreetSuggestions.length === 0 && selectedFirstStreet && (
      ...
      {selectedFirstStreet && (
        <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-800 rounded-md text-xs">
          ...
          <div className="text-gray-600 dark:text-gray-400">
            ...
            {selectedFirstStreet.lat && selectedFirstStreet.lon && 
      ...
      <div className="simulation-modal-content bg-white dark:bg-[#161B22] rounded-lg shadow-xl w-full max-w-md p-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {activeTab === "Search" && (
              <StreetSearchComponent onIntersectionSelect={handleAddIntersection} />
            )}
            {activeTab === "Map" && (
              <div className="relative">
                <MapContainer
                ...
                <div className="mt-2 space-y-1">
                  ...
                  {coordinates && (
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      <span className="font-medium">Coordinates:</span> {coordinates}
                    </p>
                  )}
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    💡 Click anywhere on the map to automatically find the nearest road intersection
                  </p>
                </div>
              </div>
            )}
