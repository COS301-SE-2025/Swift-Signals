...
const StreetSearchComponent: React.FC<{
 ...
 const [selectedFirstStreet, setSelectedFirstStreet] =
 ...
 const findIntersectingStreets = async (
  street: Street,
 ): Promise<Street[]> => {
  ...
  intersectingWays.forEach((way) => {
   if (way.tags?.name && isValidStreetName(way.tags.name)) {
    ...
    intersectingStreets.push({
     ...
     province:
      way.tags["addr:state"] || way.tags["addr:province"] || "",
     lat: centerLat,
     lon: centerLon,
     osmId: way.id,
     osmType: "way",
    ...
    if (match) {
     ...
     if (intersectingStreet && isValidStreetName(intersectingStreet)) {
      intersectingStreets.push({
       ...
       city:
        item.address?.city || item.address?.town || "South Africa",
...
const NewSimulationModal: React.FC<{
 ...
 const [activeTab, setActiveTab] = useState<"List" | "Search" | "Map">(
  "List",
 );
...
const Simulations: React.FC = () => {
 ...
 const createIntersection = async (intersectionData: {
  ...
  try {
   ...
   if (!res.ok) {
    ...
    throw new Error(
     errorData.message || "Failed to create intersection",
    );
   }
 ...
 const handleModalSubmit = async (data: {
  ...
  if (data.intersections.length === 0) {
   ...
   alert(`Failed to create ${modalType}: ${err instanceof Error ? err.message : "Unknown error"}`);
  }
 };
