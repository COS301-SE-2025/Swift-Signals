...
def test_write_node_file(no_file_write):
    ...
    no_file_write().write.assert_called_with(
        """<nodes>
...
@pytest.mark.parametrize(
    "density, expected_period",
    [
        ("low", "10"),
        ("medium", "5"),
        ("high", "2"),
        ("unknown", "5"),  # default to medium period
    ],
)
def test_generateTrips_runs_commands(monkeypatch, no_subprocess, no_makedirs, density, expected_period):
    ...
    found_period = any(
        f"--period={expected_period}" in arg or expected_period in arg
