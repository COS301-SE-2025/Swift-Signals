...
@pytest.mark.parametrize("user_inputs, expected_function", [
    (["1", "medium"], "circle"),
    (["2", "high"], "stopStreet"),
    (["3", "low"], "tJunction"),
...
def test_main_dispatch(user_inputs, expected_function, mocker):
    ...
    """
    input_mock = mocker.patch("builtins.input", side_effect=user_inputs)
...
def test_main_invalid_choice_print(mocker, capfd):
    inputs = ["invalid", "1", "medium"]
    mocker.patch("builtins.input", side_effect=inputs)
